...
class MeTableViewController: UITableViewController {
    ...
    func setUpDataArr() -> Void {
        ...
        dataArr = ["我的博客","我的微博","我的GitHub","我的简书","关于App","夜间模式","清除缓存","反馈信息","联系我们"]
    }
    
    func setupUI() -> Void {
        //背景图
        iv = UIImageView.init(frame: CGRect.init(x: 0, y: -200, width: Int( screen_width), height: 200))
        iv.image = UIImage.init(named: "手表的壁纸.jpg")
    ...
    func setUpHeadImage() -> Void {
        headImageV = UIImageView.init(frame: CGRect.init(x: 0, y: 0, width: 100, height: 100))
        headImageV.layer.cornerRadius = 50
        headImageV.layer.masksToBounds = true
        headImageV.image = UIImage.init(named: "fly.jpg")
        headImageV.center = iv.center
        self.tableView.addSubview(headImageV)
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            ...
            //关于App
            let aboutV : aboutAppTableViewController = aboutAppTableViewController()
            aboutV.hidesBottomBarWhenPushed = true
            self.navigationController?.pushViewController(aboutV, animated: true)
            ...
            let alertController = UIAlertController(title: "温馨提示:",message: "缓存文件已使用\(clear.fileSizeOfCache())M,您确定要清除缓存吗？",preferredStyle: .alert)
